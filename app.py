...
st.markdown("""<style>
    ...
    .question-list button {width: 100%; text-align: left; background: transparent; border: none; color: #d4c9bd;}
...
# ========== SESSION STATE ==========
for key in ["authenticated", "lang", "ai_response", "ai_question_input", "cached_aircraft_data", "cached_timestamp", "api_status"]:
    if key not in st.session_state:
        st.session_state[key] = False if key == "authenticated" else "" if key in ["lang", "ai_response", "ai_question_input"] else None

# ========== FULL UI DICTIONARY (add your full original if needed) ==========
UI = {
    "English": {
...
}  # ← Replace with your full original UI dict
...
Answer:"""
    ...
    try:
        completion = groq_client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": "Be precise and only use provided data."},
                      {"role": "user", "content": full_prompt}],
            temperature=0.2,
            max_tokens=600
        )
        return completion.choices[0].message.content.strip() or "No response from AI."
    except Exception as e:
...
def login_page():
    ...
    with col2:
        st.markdown("""<div class="login-container"><h2 style="text-align:center;">🌐 GlobalInternet.py Access</h2></div>""", unsafe_allow_html=True)
...
def main_page():
    ...
    with st.sidebar:
        ...
        if st.button("Refresh Data"):
            st.rerun()
        # Paste your full original sidebar here (voice buttons, range slider, status, etc.)

    # === DATA FETCHING (your original logic) ===
    if use_demo:
        aircraft_data = get_demo_aircraft() if 'get_demo_aircraft' in globals() else []
    else:
        aircraft_data, _ = fetch_live_aircraft(18.5392, -72.3364) if 'fetch_live_aircraft' in globals() else ([], "demo")
    ...
    tab_radar, tab_sat, tab_ai, tab_detect = st.tabs([L.get("radar_tab"), L.get("sat_tab"), L.get("ai_tab"), L.get("detect_tab")])

    # Paste your original Radar, Satellite, and Object Detection tabs here
    ...
    with tab_ai:
        ...
        with col_main:
            st.text_area(L.get('ai_question', 'Ask your question:'), height=120, key="ai_question_text", placeholder="Type here...")
