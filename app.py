...
def ai_analysis(aircraft, satellites, u_lat, u_lon, location_name, question=None):
    ...
    full_prompt = f"""You are an AI surveillance analyst. Ground station at {location_name} ({u_lat}, {u_lon}).
...
Answer:"""
    ...
    try:
        ...
        return response if response else "⚠️ AI returned empty response. Please try again."
    except Exception as e:
...
# ========== MAIN PAGE ==========
def main_page():
    L = UI[st.session_state.lang]   # Make sure your full UI dict is defined above

    # === SIDEBAR (add your full original sidebar code here) ===
    with st.sidebar:
        ...
        if st.button("Refresh Live Data", use_container_width=True):
            st.rerun()
        # Add the rest of your sidebar (voice, range, status, etc.)

    # Data fetching (add your original fetch logic)
    if use_demo:
    ...
    # Add your original Radar, Satellite, and Object Detection tabs here
