...
def main_page():
    sidebar_common()
    # Logout button only once in sidebar
    if st.sidebar.button(_("logout_button"), key="logout_main", use_container_width=True):
        st.session_state.authenticated = False
        st.rerun()
    ...
    with tab2:
        demo_sat = st.sidebar.checkbox(_("demo_mode_satellite"), key="demo_sat_checkbox", value=False)
